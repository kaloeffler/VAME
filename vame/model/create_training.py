...
def traindata(cfg, files, testfraction, num_features, savgol_filter):
    ...
    np.save(
        os.path.join(cfg["project_path"], "data", "train", "normalize_mean.npy"), X_mean
    )
    np.save(
        os.path.join(cfg["project_path"], "data", "train", "normalize_std.npy"), X_std
    )
...
def traindata_legacy(cfg, files, testfraction, num_features, savgol_filter):
    ...
    if savgol_filter:
        X_med = scipy.signal.savgol_filter(
            seq_inter, cfg["savgol_length"], cfg["savgol_order"]
        )
    num_frames = len(X_med.T)
    test = int(num_frames * testfraction)
