...
def random_generative_samples_motif(cfg, model, latent_vector, labels, n_cluster):
    ...
    for j in range(n_cluster):
        ...
        tensor_sample = (
            torch.from_numpy(density_sample[0]).type("torch.FloatTensor").cuda()
        )
...
def random_reconstruction_samples(cfg, model, latent_vector):
    ...
    tensor_sample = (
        torch.from_numpy(latent_vector[rnd]).type("torch.FloatTensor").cuda()
    )
    decoder_inputs = tensor_sample.unsqueeze(2).repeat(1, 1, time_window)
    decoder_inputs = decoder_inputs.permute(0, 2, 1)
    ...
    for file in files:
        ...
        if mode == "motifs":
            ...
            random_generative_samples_motif(
                cfg, model, latent_vector, labels, n_cluster
            )
