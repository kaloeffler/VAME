...
def get_labels(cfg, files, model_name, n_cluster):
    ...
    for file in files:
        ...
        label = np.load(
            os.path.join(path_to_file, str(n_cluster) + "_km_label_" + file + ".npy")
        )
        labels.append(label)
    return labels
...
def umap_vis(cfg, file, embed, community_labels_all):
    ...
    plt.colorbar(boundaries=np.arange(np.max(num) + 2) - 0.5).set_ticks(
        np.arange(np.max(num) + 1)
    )
...
def community(config, show_umap=False, cut_tree=None):
    ...
    for idx, file in enumerate(files):
        ...
        np.save(
            os.path.join(
                path_to_file, "community", "transition_matrix_" + file + ".npy"
            ),
        ...
        with open(
            os.path.join(path_to_file, "community", "hierarchy" + file + ".pkl"), "wb"
        ) as fp:  # Pickling
