...
def create_video(
    path_to_file, file, embed, clabel, frames, start, length, max_lag, num_points
):
...
def gif(
    ...
    for file in files:
        ...
        embed = np.load(
            os.path.join(path_to_file, "community", "umap_embedding_" + file + ".npy")
        )

        try:
            embed = np.load(
                os.path.join(
                    path_to_file, "", "community", "", "umap_embedding_" + file + ".npy"
                )
            )
        ...
        if label == "motif":
            umap_label = np.load(
                os.path.join(
                    path_to_file, str(n_cluster) + "_km_label_" + file + ".npy"
                )
            )
        elif label == "community":
            umap_label = np.load(
                os.path.join(
                    path_to_file, "community", "community_label_" + file + ".npy"
                )
            )
        elif label == None:
            umap_label = None
