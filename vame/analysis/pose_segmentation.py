...
def same_parameterization(cfg, files, latent_vector_files, cluster):
    ...
    kmeans = KMeans(
        init="k-means++", n_clusters=cluster, random_state=random_state, n_init=n_init
...
def pose_segmentation(config):
    ...
    if legacy == True:
        ...
        behavior_segmentation(
            config, model_name=model_name, cluster_method="kmeans", n_cluster=n_cluster
        )
        ...
        if new == True:
            for idx, file in enumerate(files):
                ...
                np.save(
                    os.path.join(save_data, str(n_cluster) + "_km_label_" + file),
                    labels[idx],
                )
                np.save(
                ...
                np.save(
                    os.path.join(save_data, "motif_usage_" + file), motif_usages[idx]
                )
