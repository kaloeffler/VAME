...
def kmeans_clustering(context, n_clusters):
    kmeans = KMeans(
        init="k-means++", n_clusters=n_clusters, random_state=42, n_init=15
    ).fit(context)
...
def behavior_segmentation(
    config, model_name=None, cluster_method="kmeans", n_cluster=[30]
):
...
def cluster_latent_space(
    cfg, files, z_data, z_logger, cluster_method, n_cluster, model_name
):
    ...
    for cluster in n_cluster:
        if cluster_method == "kmeans":
            ...
            data_labels = np.int64(
                scipy.signal.medfilt(data_labels, cfg["median_filter"])
            )
